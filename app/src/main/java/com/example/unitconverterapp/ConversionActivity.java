...
public class ConversionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayAdapter<UnitFunctions.SelectionOption> adp = null;
        ...
        for(int i =0;i<dataStr.length;i++){
            selectionOptions.add(new UnitFunctions().new SelectionOption(dataStr[i],dataInt[i]));
        }
        adp = new ArrayAdapter<UnitFunctions.SelectionOption>(this,
                android.R.layout.simple_spinner_dropdown_item,selectionOptions);
    ...
    public void calculationClick(View v){
        if(!txtValue.getText().equals("")){
            UnitFunctions.SelectionOption option =
                    (UnitFunctions.SelectionOption)spSelection.getSelectedItem();
            double result = UnitFunctions.makeGenericConversion(option.getFunction(),
                    Double.valueOf(txtValue.getText().toString()));
